...
import yksg.kuangkuang.main.EnterActivity;

/**
 * 时间工具（下面好多方法都判断过8小时的问题了）
 */
...
public class MyDate {
    ...
    public static String yearmonthDay(String time) {
        ...
        if (time.contains("Z")){
            try {
                right_time=formatTimeEight(time.substring(0, 19).replace("T", " "));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }else{
            right_time=time.substring(0, 19).replace("T", " ");
        }
        return right_time.substring(0, 10);
    }
    ...
    public static String monthDay(String time) {
        ...
        return right_time.substring(5, 16);
    }
    ...
    public static String month(String time) {
        ...
        String the_time = right_time.substring(5, 7);
        String month;
    ...
    public static String Day(String time) {
        ...
        return right_time.substring(8, 10);
    }
     ...
     * @return
     */
    public static String timeLogic(String dateStr) throws Exception {
        String right_time;
        if (dateStr.contains("Z")){
            right_time=formatTimeEight(dateStr.substring(0, 19).replace("T", " "));
        }else{
            right_time=dateStr.substring(0, 19).replace("T", " ");
        }
        //我知道我写的特别low，但时间太紧，不爱动脑啊，先这样吧，以后有时间再优化
        int year = Integer.parseInt(right_time.substring(0, 4));//数据的年份
        int this_year = Calendar.getInstance().get(Calendar.YEAR);//当前年份
        StringBuffer sb = new StringBuffer();
        if (year == this_year) {
            int ct = 3600 * 24;
            String the_time = timeStringToUnixTime123(right_time);
            // 相差的秒数
            ...
            if (time > 0 && time < 60) { // 1小时内
                ...
                return sb.append(yearmonthDay(right_time)).toString();
            }
